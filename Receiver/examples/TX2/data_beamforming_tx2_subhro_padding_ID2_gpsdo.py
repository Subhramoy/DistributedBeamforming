...
class data_beamforming_tx2(gr.top_block):
    ...
    def __init__(self):
        ...
        self.beamforming_matlab_file_payload_py_0_0 = beamforming.matlab_file_payload_py('/home/genesys/workarea-gnuradio/gnuradio/gr-beamforming/examples/data/trainingSig2')
...
#	try:
	...
	###############################
	#check for gps lock
	gps_lock=self.uhd_usrp_sink_0.get_mboard_sensor("gps_locked",0)
	#check for ref lock
	ref_lock=self.uhd_usrp_sink_0.get_mboard_sensor("ref_locked",0)
	...
	if gps_lock.to_bool() == False:
		...
		last_pps_time = self.uhd_usrp_sink_0.get_time_last_pps()
        	while last_pps_time.get_real_secs() == self.uhd_usrp_sink_0.get_time_last_pps().get_real_secs():
			#time.sleep(0.1)
			print "USRP PPS time syncing ..."		
		#get current gps_time
		gps_time=self.uhd_usrp_sink_0.get_mboard_sensor("gps_time",0)
		#set the gps_time+1 on the next pps edge
		self.uhd_usrp_sink_0.set_time_next_pps(uhd.time_spec(gps_time.to_int()+1))
		last_pps_time = self.uhd_usrp_sink_0.get_time_last_pps()
	...
	#self.uhd_usrp_source_0.set_time_next_pps(uhd.time_spec_t(system_time +1))
		self.uhd_usrp_sink_0.set_start_time(uhd.time_spec_t(gps_time.to_int() + wait_time))
		print "wait over"
	lock.release()
